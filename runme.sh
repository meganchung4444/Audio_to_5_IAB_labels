...
CUDA_VISIBLE_DEVICES=0 python3 /content/panns_transfer_to_gtzan/pytorch/main.py train --training_dataset_dir=$TRAINING_DATASET_DIR --val_dataset_dir=$VAL_DATASET_DIR --test_dataset_dir=$TEST_DATASET_DIR --workspace=$WORKSPACE --holdout_fold=1 --model_type="Transfer_Cnn14" --pretrained_checkpoint_path=$PRETRAINED_CHECKPOINT_PATH --loss_type=clip_nll --augmentation='none' --learning_rate=1e-4 --batch_size=32 --max_epoch=100 --cuda
